...
const verifyPassword = (password, stored) => {
  ...
  if (scheme !== "scrypt" || !salt || !hash) {
    // Legacy unsalted SHA-256 hashes fall back here; they still verify but
    // should be re-saved (re-registered or password changed) to upgrade.
...
exports.createOrder = async event => {
  ...
  try {
    ...
    const order = { ...orderKey(orderId), orderId, customerName: String(input.customerName), items, total: total / 100, status: "Confirmed", paymentMethod: String(input.paymentMethod || "Card"), paymentAmount: Number(input.paymentAmount), date: new Date().toISOString() };
