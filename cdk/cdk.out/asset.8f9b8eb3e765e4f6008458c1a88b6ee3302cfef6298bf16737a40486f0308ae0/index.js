...
exports.getProducts = async () => {
  try { return response(200, await products()); }
...
exports.createProduct = async event => {
  ...
  const product = {
    ...
    description: String(input.description || ""), price: Number(input.price), stock: Number(input.stock), category: String(input.category)
  };
...
exports.login = async event => {
  const input = bodyOf(event) || {};
  if (input.username !== process.env.ADMIN_USERNAME || input.password !== process.env.ADMIN_PASSWORD) return response(401, { success: false, message: "Invalid username or password" });
  return response(200, { success: true, message: "Admin login successful", admin: { name: "Administrator", email: process.env.ADMIN_EMAIL || "" } });
};
...
exports.listOrders = async () => {
  try {
    const result = await client.send(new QueryCommand({ TableName: TABLE_NAME, KeyConditionExpression: "entityType = :type", ExpressionAttributeValues: { ":type": "ORDER" } }));
