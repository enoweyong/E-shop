...
S3Client.prototype.send = async function(command) {
  ...
  if (command instanceof DeleteObjectCommand) {
    s3DeletedKeys.push(command.input.Key);
    return {};
  }
...
async function runLambdaTests() {
  ...
  const createRes = await lambda.catalogRouter({
    httpMethod: "POST",
    headers: authHeader,
    body: JSON.stringify({ productId: 1, name: "Test Laptop", price: 500, stock: 10, category: "Tech", imageUrl: base64Img, isPublic: true })
  });
  ...
  // Create Product 2 (Private)
  const createPrivateRes = await lambda.catalogRouter({
    httpMethod: "POST",
    headers: authHeader,
    body: JSON.stringify({ productId: 2, name: "Private Laptop", price: 800, stock: 5, category: "Tech", isPublic: false })
  });
  assert.strictEqual(createPrivateRes.statusCode, 201);

  // Unauthenticated Get Product List -> Should return ONLY Public product (1)
  const listRes = await lambda.catalogRouter({ httpMethod: "GET" });
  assert.strictEqual(listRes.statusCode, 200);
  const publicProducts = JSON.parse(listRes.body);
  assert.strictEqual(publicProducts.length, 1);
  assert.strictEqual(publicProducts[0].productId, 1);

  // Authenticated Admin Dashboard -> Should return both public and private products (2)
  const adminListRes = await lambda.catalogRouter({ httpMethod: "GET", headers: authHeader });
  assert.strictEqual(adminListRes.statusCode, 200);
  const adminProducts = JSON.parse(adminListRes.body);
  assert.strictEqual(adminProducts.length, 2);
  ...
  // 4. Delete Product & Verify S3 deletion
  console.log("4. Testing Product Deletion & S3 Cleanup");
  ...
  assert.ok(s3DeletedKeys.length > 0, "S3 image key should be deleted");

  const getDeletedRes = await lambda.getProduct({ pathParameters: { id: "1" } });
  assert.strictEqual(getDeletedRes.statusCode, 404);
  console.log("✓ Product deletion & S3 cleanup test passed");
